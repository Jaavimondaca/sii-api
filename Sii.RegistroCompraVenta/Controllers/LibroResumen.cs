...
public class LibroResumen : Controller
{
    ...
    [SwaggerOperation(
        Summary = "Obtiene el detalle del libro de compras o ventas (facturas individuales)",
        Description = "Devuelve la lista detallada de facturas para un tipo de documento específico."
    )]
    [HttpGet("detalle")]
    public async Task<IActionResult> GetDetalle(
        ...
        [SwaggerParameter("Mes del período tributario. Valores de 1 a 12.")] [FromQuery] int? mes,
        [SwaggerParameter("Tipo de operación: COMPRA o VENTA.")] [FromQuery] string? operacion,
        [SwaggerParameter("Código del tipo de documento (ej: 33 para Factura Electrónica).")] [FromQuery] string? tipoDoc
    )
    {
        object validacion = ValidarParametros(rut, year, mes, operacion);
        if (validacion is string error)
        ...
        if (string.IsNullOrWhiteSpace(tipoDoc))
            return BadRequest("El parámetro 'tipoDoc' es obligatorio.");
        ...
        try
        {
            JsonElement data = await libroCompra.GetDetalle(
                rutOk,
                periodoOk,
                operacionOk,
                tipoDoc.Trim()
